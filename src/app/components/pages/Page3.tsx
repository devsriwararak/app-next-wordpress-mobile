...
const Page3 = ({ setStatusForm }: { setStatusForm: (value: boolean) => void }) => {
    ...
    const handleCalculate = () => {
        ...
        if (select1 && select2 && select3 && select4 && select5) {
            sum = (Number(select1) - Number(select2)) - Number(select3) - Number(select4) - Number(select5)
            if (select6) {
            ...
            <p className='my-4 bg-yellow-400'>รอถามลูกค้าว่า จะเอาค่าความเสียหายกี่ช่อง อะไรบ้าง ?</p>
                ...
                <div className='mt-8 text-center'>
                    <h2 className='text-sm text-red-500'>
                        * ยอดดังกล่าวเป็นเพียงการประเมิณคร่าวๆเท่านั้น ( ต้องให้เจ้าหน้าที่ประเมินอีกครั้งหนึ่งที่หน้าร้าน )
                    </h2>
                </div>
            </section>
